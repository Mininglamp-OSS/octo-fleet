...
-- time from device_info.device_id (stored as device.device_uuid). One device
-- may host many daemon_ids, so multiple agent_runtime rows can share one
-- device_id (many-to-one). 0 = not yet linked (pre-backfill rows); backfill
-- populates it later. Kept out of the unique key so the upsert path does not
-- depend on a device row already existing.
